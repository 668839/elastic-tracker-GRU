/*
    FILE: uvDetector.h
    ------------------
    helper class function definitions for uv detector(用于紫外线探测器的辅助类函数定义)

    1.它处理深度图像和RGB图像，生成U-depth map，并从中提取障碍物的2D和3D边界框。
    2.此外，它还集成了跟踪功能来估计障碍物的状态。
    代码中还涉及辅助类UVbox（用于表示边界框）和UVtracker（用于跟踪），但用户问题聚焦于U-depth检测，因此主要分析UVdetector类的函数
*/
...
namespace onboardDetector{
    /*
    默认构造函数：UVbox::UVbox()
            ​作用​​：初始化一个空的UVbox对象，将所有成员设置为默认值。这用于创建未定义的框实例，通常作为占位符或初始状态。
    */
    UVbox::UVbox()
    {
        this->id = 0;   // 将id设置为0，表示无效或未初始化的框
        this->toppest_parent_id = 0;    // 将最顶层父ID设置为0，表示无父框
        this->bb = cv::Rect(cv::Point2f(0, 0), cv::Point2f(0, 0)); // 初始化bb为一个空矩形：左上角点(0,0)，右下角点(0,0)，宽度和高度均为0
    }

    /*
    ​带参数构造函数 作用​​：根据U-map中的线段信息创建UVbox实例。线段来自U-map的某一行，表示该行上一个连续的高值区域（可能属于一个障碍物）。
                    此构造函数初始化框的基本属性，并设置矩形框的初始形状（高度为1行，宽度由left和right定义）。
    */
    ...
    /*
     合并函数：​作用​​：合并两个UVbox对象（“父框”和“子框”），生成一个更大的框，覆盖两个框的联合区域。这用于处理U-map中跨行的障碍物，将同一障碍物的多行线段合并为单个边界框。
    */
